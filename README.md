...
[![Version](https://img.shields.io/badge/version-0.1.4-orange)](https://github.com/FROSADO/onecloudriver/releases/tag/v0.1.4)
...
wget https://github.com/FROSADO/onecloudriver/releases/download/v0.1.4/onecloudriver_linux_amd64.zip
...
wget https://github.com/FROSADO/onecloudriver/releases/download/v0.1.4/onecloudriver_0.1.4_amd64.deb
sudo dpkg -i onecloudriver_0.1.4_amd64.deb
```
...
wget https://github.com/FROSADO/onecloudriver/releases/download/v0.1.4/onecloudriver-0.1.4-1.x86_64.rpm
```
...
sudo dnf install ./onecloudriver-0.1.4-1.x86_64.rpm

# RHEL / CentOS 7 (older)
sudo yum install ./onecloudriver-0.1.4-1.x86_64.rpm

# openSUSE
sudo zypper install ./onecloudriver-0.1.4-1.x86_64.rpm
```
...
gh release download v0.1.4 --repo FROSADO/onecloudriver
...
```bash
# Install service (auto-detects account if only one exists; a missing
# mountpoint directory is created automatically during install)
onecloudriver service install --mountpoint /home/<user>/OneDrive/%i

# Install and enable for specific account
onecloudriver service install --mountpoint /home/<user>/OneDrive/%i -a user@outlook.com --enable

# Install for ALL accounts
onecloudriver service install --mountpoint /home/<user>/OneDrive/%i --all --enable

# Manage
onecloudriver service list                 # View all installed instances
onecloudriver service status              # View status and failure journal
...
> 🛠️ **Troubleshooting `203/EXEC`** — If an instance fails to start with
> `203/EXEC` (`systemctl --user status onecloudriver@<account>`), the
> `ExecStart` binary path in the unit is stale (e.g. it was generated from a
> temporary `go test` binary). Reinstall the service through the installed
> binary: `onecloudriver service uninstall --all` then
> `/usr/local/bin/onecloudriver service install -a user@outlook.com`.
> See [the manual](docs/MANUAL.md#troubleshooting-203exec).
