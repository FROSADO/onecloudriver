...
Usage:
  fuse_bench.py --binary PATH --mount MP --cache-dir DIR
                [--iters N] [--cold-count N] [--out results.json]
                [--keep] [--dry-run]
"""
import argparse
import json
...
def find_daemon_pid():
    """Return the PID of the process holding a /dev/fuse fd (the mount daemon)."""
...
def mount(binary, mp, cache_dir, log_path, mount_extra=None):
...
def bench(binary, mp, cache_dir, iters, cold_count, out_path, describe):
    # scan /proc for the daemon whose cmdline references our mountpoint
    for cand in os.listdir("/proc"):
        if not cand.isdigit():
            continue
        try:
            with open(f"/proc/{cand}/cmdline", "rb") as f:
                cl = f.read().decode(errors="replace")
        except OSError:
            continue
        if mp in cl and "mount" in cl:
            pid = int(cand)
            break
    if pid is None:
        pid = find_daemon_pid()
    if pid is None:
        sys.exit(f"ERROR: could not find FUSE daemon for {mp}")
...
def main():
    ...
    if any(a.mount in l for l in open("/proc/mounts")):
        unmount(a.mount)
        time.sleep(1.0)
    mount(a.binary, a.mount, a.cache_dir, a.log)
    desc = {"binary": a.binary, "mount": a.mount, "cache_dir": a.cache_dir}
    try:
        bench(a.binary, a.mount, a.cache_dir, a.iters, a.cold_count, a.out, desc)
    finally:
        if not a.no_unmount:
            unmount(a.mount)
