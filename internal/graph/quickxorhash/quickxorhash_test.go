...
func TestQuickXorHash(t *testing.T) {
	for _, test := range testVectors {
		...
		if err != nil {
			t.Fatalf("%s: decode input: %v", what, err)
		}
		got := Sum(in)
		want, err := base64.StdEncoding.DecodeString(test.out)
		if err != nil {
			t.Fatalf("%s: decode want: %v", what, err)
		}
		if !bytes.Equal(got[:], want) {
			t.Errorf("%s: got %s, want %s", what, base64.StdEncoding.EncodeToString(got[:]), test.out)
		}
	}
}
	...
	for _, blockSize := range []int{1, 2, 4, 7, 8, 16, 32, 64, 128, 256, 512} {
		for _, test := range testVectors {
			...
			for i := 0; i < len(in); i += blockSize {
				...
				if n != end-i {
					t.Fatalf("%s: wrote %d, want %d", what, n, end-i)
				}
			}
			got := h.Sum(nil)
			want, err := base64.StdEncoding.DecodeString(test.out)
...
func TestSize(t *testing.T) {
	if got, want := New().Size(), 20; got != want {
		t.Errorf("Size() = %d, want %d", got, want)
	}
}

func TestBlockSize(t *testing.T) {
	if got, want := New().BlockSize(), 64; got != want {
...
func TestReset(t *testing.T) {
	...
	if bytes.Equal(zeroHash, d.Sum(nil)) {
